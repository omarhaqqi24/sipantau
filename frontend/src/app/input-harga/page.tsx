...
export default function InputHarga() {
  ...
  const [hargaData, setHargaData] = useState(
    Array.from({ length: 30 }, () => ({ harga: '', tanggal: null as Date | null }))
  )
  ...
  const handleSubmit = async () => {
    ...
    const validItems = hargaData
      ...
      .map(item => ({
        harga_petani: parseInt(item.harga.toString()),
    ...
    try {
      const response = await fetch('/api/harga-petani', {
        method: 'POST',
      ...
      if (response.ok && data.success) {
        setPopup({ type: 'success', message: 'Data harga berhasil disimpan!' })
        // Clear form
